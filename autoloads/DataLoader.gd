...
func _make_fallback_player(index: int) -> PlayerData:
	...
	data.acceleration_time = 0.22 # FM2D compromise: central mid acceleration baseline
	data.friction_time = 0.12 # FM2D compromise: snappier stop response while retaining inertia
	data.turning_penalty = 0.35 # FM2D compromise: balanced turning penalty factor
...
func _build_nordvik() -> TeamData:
	...
	team.squad = [
		_player(1, "Mads Dahl", "GK", 82.0, 175.0, 0.32, 0.12, 0.50, 1.30, 0.80, 0.85, 0.30),
		_player(5, "Halvard Brann", "CB", 92.0, 185.0, 0.32, 0.12, 0.50, 1.25, 0.60, 0.75, 0.70),
		_player(4, "Sigurd Voss", "CB", 88.0, 188.0, 0.32, 0.12, 0.50, 1.28, 0.65, 0.70, 0.65),
		_player(3, "Erik Lund", "LB", 77.0, 210.0, 0.22, 0.12, 0.35, 1.42, 0.70, 0.60, 0.55),
		_player(2, "Torben Hauge", "RB", 76.0, 212.0, 0.22, 0.12, 0.35, 1.43, 0.72, 0.62, 0.58),
		_player(6, "Niklas Borg", "DM", 80.0, 200.0, 0.32, 0.12, 0.50, 1.35, 0.82, 0.78, 0.72),
		_player(8, "Olav Strand", "CM", 74.0, 208.0, 0.22, 0.12, 0.35, 1.40, 0.85, 0.72, 0.60),
		_player(10, "Petter Naess", "AM", 71.0, 215.0, 0.15, 0.12, 0.25, 1.48, 0.90, 0.80, 0.55),
		_player(11, "Jonas Elv", "LW", 69.0, 228.0, 0.15, 0.12, 0.25, 1.52, 0.75, 0.55, 0.62),
		_player(7, "Rune Kval", "RW", 70.0, 225.0, 0.15, 0.12, 0.25, 1.50, 0.73, 0.58, 0.65),
		_player(9, "Henrik Ask", "ST", 83.0, 220.0, 0.22, 0.12, 0.35, 1.50, 0.68, 0.65, 0.88),
	]
	return team
...
func _build_solano() -> TeamData:
	...
	team.squad = [
		_player(1, "Carlos Vega", "GK", 80.0, 172.0, 0.32, 0.12, 0.50, 1.28, 0.78, 0.88, 0.25),
		_player(5, "Mateo Ruiz", "CB", 87.0, 190.0, 0.32, 0.12, 0.50, 1.27, 0.62, 0.72, 0.68),
		_player(4, "Diego Pons", "CB", 90.0, 186.0, 0.32, 0.12, 0.50, 1.26, 0.58, 0.68, 0.72),
		_player(3, "Luis Ferrer", "LB", 74.0, 218.0, 0.22, 0.12, 0.35, 1.48, 0.74, 0.64, 0.52),
		_player(2, "Andres Mora", "RB", 75.0, 216.0, 0.22, 0.12, 0.35, 1.46, 0.71, 0.62, 0.54),
		_player(6, "Pablo Cano", "DM", 79.0, 202.0, 0.32, 0.12, 0.50, 1.36, 0.80, 0.76, 0.75),
		_player(8, "Rafael Soto", "CM", 73.0, 212.0, 0.22, 0.12, 0.35, 1.42, 0.83, 0.74, 0.58),
		_player(10, "Marco Reyes", "AM", 70.0, 218.0, 0.15, 0.12, 0.25, 1.50, 0.92, 0.82, 0.52),
		_player(11, "Javier Tur", "LW", 67.0, 258.0, 0.15, 0.12, 0.25, 1.58, 0.65, 0.45, 0.70),
		_player(7, "Ivan Blasco", "RW", 68.0, 245.0, 0.15, 0.12, 0.25, 1.55, 0.68, 0.50, 0.68),
		_player(9, "Bruno Tena", "ST", 81.0, 222.0, 0.22, 0.12, 0.35, 1.52, 0.70, 0.68, 0.85),
	]
	return team
