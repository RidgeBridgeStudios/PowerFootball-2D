...
Status of this document: **design, not yet implemented**, except where a section is explicitly marked `[EXISTS]`. Read [CORE_INVARIANTS.md](CORE_INVARIANTS.md) and `.claude/rules/ai-architect.md` before touching any file named here — every new hook below is designed to slot into an existing choke point rather than add a new one.

---

## 0. Ground Truth — What Already Exists vs. What This Document Adds

`ROADMAP.md` Phase 2 currently shows the trait bitmask, `overall_rating`, and `reputation` as unchecked `[ ]`. That is stale relative to the code. The actual state, verified against source on 2026-09-08:

| Component | File | State |
|---|---|---|
| Player trait bitmask (13 flags) | `shared/PlayerData.gd:75-89` | **`[EXISTS]`** — data only. `has_trait(bit)` is called from exactly one production call site (`MoraleEngine.gd:110`, `PressureImmune` damping a mood seed). The other 12 traits and every in-match consumer are unwritten. |
| `player_reputation`, `calculate_overall_rating()`, `calculate_market_value()` | `shared/PlayerData.gd:92,197,259` | **`[EXISTS]`** — derived fields are already computed and already feed transfer value. Nothing reads `overall_rating` to gate AI behavior (star-marking) yet. |
