...
|---|---|---|
| `autoloads/MatchWorldModel.gd` | ALL spatial position reads route here. Global cache of 22 players + ball. Owns `defensive_line_x` — shared per-team defensive-line depth (world X), recomputed every physics frame from ball position + carrier pressure. Defenders blend toward this instead of computing independent lines. | Call `get_tree().get_nodes_in_group()` inside `_process`/`_physics_process`; compute a per-player defensive line independently instead of reading `defensive_line_x`. |
...
| `shared/CollisionLayers.gd` | Layer bitmask constants. Layer 1 = World/Pitch bounds, Layer 2 = Players, Layer 3 = Ball, Layer 4 = Hitboxes/Sensors. CharacterBody2D masks Layer 1+2 ONLY. | Mask Layer 3 in CharacterBody2D (ball manages its own pseudo-3D height and collision). |
