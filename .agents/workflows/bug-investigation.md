...
1. **Symptom Triage & Invariant Classification:**
   ...
   - Check layer rules via `python3 tools/layer_context.py [1-5]` or `python3 tools/semantic_search.py "<symptom>"`.
   - Distinguish defects from pending roadmap items via `python3 tools/next_task.py`.
...
2. **Targeted Errata Reconnaissance (Zero Full Dumps):**
   ...
   - Search historical failure modes with `python3 tools/semantic_search.py "<subsystem> <symptom>"`.
...
3. **Structural Root-Cause Tracing:**
   ...
   - Trace callers and blast radius: `python3 tools/dump_dep_graph.py --blast-radius <target>`.
...
4. **Targeted Code Inspection:**
   - Never read whole files. Extract specific functions, enums, or headers via:
     `python3 tools/codebase_slice.py <file> --func <name>` (or `--class-header`, `--signals`).
...
6. **Execution & Regression Verification:**
   ...
   - If non-obvious lesson learned, record structured errata in `AGENTS_ERRATA.md` and run `python3 tools/sync_rules.py`.
