...
1. **Context & Inspection Hygiene (Zero File Dumps):**
   ...
   - Do NOT dump `docs/json-schema.md` (410 lines). Query targeted sections via:
     `python3 tools/semantic_search.py "<attribute_or_entity>"`
...
2. **Schema Invariants & Blast Radius (Pre-Edit):**
   ...
   - Before proposing schema changes, compute consumer blast radius:
     `python3 tools/dump_dep_graph.py --blast-radius autoloads/DataLoader.gd`
...
3. **Data Generation & Modification:**
   ...
   - Re-generate data files via:
     `python3 tools/update_db_all.py || python tools/update_db_all.py || py -3 tools/update_db_all.py`
     or `python3 tools/expand_database.py || python tools/expand_database.py || py -3 tools/expand_database.py`
