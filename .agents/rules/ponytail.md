...
2. **Codebase Reuse** — Route spatial reads through `autoloads/MatchWorldModel.gd` and inter-system events through `autoloads/GameEvents.gd`. Never write a parallel spatial query or a second event bus. For cross-agent shared memory, use the existing `AGENTS_ERRATA.md` structured-YAML mechanism (Section 10 of `AGENTS.md`) instead of standing up a new database.
...
4. **Native Node Architecture** — Rely on the declarative 22-player scene layout in `pitch/PitchScene.tscn` and the existing collision matrix (`shared/CollisionLayers.gd`); `CharacterBody2D` masks Layer 1 + 2 only.
5. **Existing Invariant Scripts** — Check `shared/UtilityMath.gd`, `shared/FormationAnchorMath.gd`, and `docs/MATH_SOLVERS.md` before deriving a new formula; the closed-form solver you need may already be there.
